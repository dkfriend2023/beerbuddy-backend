...
class sms(APIView):
    def send_sms(self, phone_number, auth_number):
        ...
        headers = {
            ...
            # (예시 = 'nCH86JcJ6FCl40eYc4qp')
            'x-ncp-iam-access-key': 'tXFTpMViHVCr8WWM4tVL',
            # utils.py 이용
        ...
        requests.post(SEND_URI, headers=headers, data=body)
    ...
    def post(self, request, *args, **kwargs):
        ...
        try:
            ...
            print("done")
            return JsonResponse({'message': '인증번호 발송완료', 'auth_num': auth_num}, status=200)
            ...
            self.send_sms(phone_number=input_mobile_num, auth_number=auth_num)
            return JsonResponse({'message': '인증번호 발송 및 DB 입력완료', 'auth_num': auth_num}, status=200)
